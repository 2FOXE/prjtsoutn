...
const ClientList = () => {
  ...
  const [showEditModalregionsSite, setShowEditModalregionsSite] =
    useState(false);
  ...
  const fetchClients = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8000/api/all-data-client"
      );
      ...
      localStorage.setItem(
        "siteClients_societe",
        JSON.stringify(data.site_clients || [])
      );
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (storedSecteurClients)
      setSecteurClient(JSON.parse(storedSecteurClients));
  ...
  const handleEdit = (client) => {
    ...
    setSelectedProductsData(
      client.contact_clients?.map((contact) => ({ ...contact }))
    );
    setSelectedProductsDataRep(
      client.represantant?.map((contact) => ({ ...contact }))
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const formDataRep = {
        represantants: selectedProductsDataRep
          .filter((contact) => contact.id_agent && contact.id_agent !== "")
          ?.map((contact) => ({
            ...contact,
      ...
      if (
        selectedProductsDataRep.filter(
          (contact) => contact.id_agent && contact.id_agent !== ""
        )?.length > 0
      ) {
      ...
      if (response.status === 200) {
        ...
        setSelectedProductsData([]);
        console.log(
          "setSelectedProductsData: " + JSON.stringify(selectedProductsData)
        );
        setSelectedProductsDataRep([]);
        setFormData({
          CodeClient: "",
  ...
  const handleEditSC = (siteClient) => {
    ...
    setSelectedProductsData(
      siteClient.contact_site_clients?.map((contact) => ({ ...contact }))
    );
    setSelectedProductsDataRep(
      siteClient.represantant?.map((contact) => ({ ...contact }))
    );
  ...
  const handleSubmitSC = async (e) => {
    ...
    if (editingSiteClient) {
      ...
      if (formDataSC.logoSC) formDataScd.append("logoSC", formDataSC.logoSC);
  ...
  const handleSelectAllChange = () => {
    ...
    const clientsOnCurrentPage = filteredClients.slice(
      page * rowsPerPage,
    ...
    if (newSelectAll) {
      ...
      setSelectedItems((prevSelectedItems) =>
        prevSelectedItems.filter(
          (id) => !clientsOnCurrentPage.some((client) => client.id === id)
        )
      );
    }
  };
  ...
  const handleCheckboxChange = (itemId) => {
    ...
    setSelectAll(
      clientsOnCurrentPage.every((client) =>
        updatedSelectedItems.includes(client.id)
      )
    );
  };
            ...
            <tbody>
              ${filteredClients
                ...
                <tr>
                  <td><img src="${
                    client.logoC
                      ? `http://localhost:8000/storage/${client.logoC}`
                      : "http://localhost:8000/storage/default_user.png"
                  }" loading="lazy" alt="Logo" style="width:50px; height:50px; border-radius:50%;" /></td>
  ...
  const handleDeleteProduct = (index, id) => {
    ...
    if (id) {
      axios.delete(`http://localhost:8000/api/contactClient/${id}`).then(() => {
        fetchClients();
      });
    }
  };
  ...
  const filteredClients = clients.filter((client) => {
    return (
      ...
      (selectedCategory ? client.secteur_id === selectedCategory : true) &&
      // (searchTerm ? client?.secteur?.secteurClient?.toLowerCase().startsWith(searchTerm.toLowerCase()) : true) ||
      ((searchTerm
        ? client?.zone?.zone?.toLowerCase().includes(searchTerm.toLowerCase())
        : true) ||
        (searchTerm
          ? client?.region?.region
              ?.toLowerCase()
        ...
        (searchTerm
          ...
          : true) ||
        (searchTerm
          ? client?.categorie?.toLowerCase().includes(searchTerm.toLowerCase())
          : true) ||
        (searchTerm
        ...
        (searchTerm
          ...
          : true) ||
        (searchTerm
          ? client?.seince?.toLowerCase().includes(searchTerm.toLowerCase())
          : true) ||
        (searchTerm
          ? client?.abreviation
              ?.toLowerCase()
          ...
          : true) ||
        (searchTerm
          ? client?.tele?.toLowerCase().includes(searchTerm.toLowerCase())
          : true) ||
        (searchTerm
          ? client?.code_postal
              ?.toLowerCase()
        ...
        (searchTerm
          ? modePaimant
              .find((agent) => agent.id === client?.mod_id)
              ?.mode_paimants.toLowerCase()
              .includes(searchTerm.toLowerCase())
          : true) ||
        (searchTerm
          ? secteurClient
              .find((agent) => agent.id === client?.secteur_id)
              ?.secteurClient?.toLowerCase()
              .includes(searchTerm.toLowerCase())
          : true))
    );
  });
  ...
  const handleSave = async () => {
    try {
      await axios.put(`http://localhost:8000/api/zones/${categorieId}`, {
        zone: selectedCategoryId,
      });
      await fetchClients(); // Refresh categories after adding
  ...
  const handleSaveRegine = async () => {
    try {
      await axios.put(`http://localhost:8000/api/regions/${categorieId}`, {
        region: selectedCategoryId,
      });
      await fetchClients(); // Refresh categories after adding
  ...
  const handleEditRegine = (categorieId) => {
    setErrors({});
    setSelectedCategoryId(categorieId.region);
    setCategorie(categorieId.id);
  ...
  const handleAddSecteur = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:8000/api/secteur_clients",
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  const handleSaveSecteur = async () => {
    try {
      ...
      if (newSecteur.logoP) formDataSecteur.append("logoP", newSecteur.logoP);
      formDataSecteur.append("_method", "put");
      formDataSecteur.append("secteurClient", newSecteur.secteurClient);
      await axios.post(
        `http://localhost:8000/api/secteur_clients/${categorieId}`,
        formDataSecteur,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  const handleAddModP = async () => {
    try {
      ...
      const response = await axios.post(
        "http://localhost:8000/api/mode-paimants",
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
  ...
  const handleSaveModP = async () => {
    try {
      await axios.put(
        `http://localhost:8000/api/mode-paimants/${categorieId}`,
        { mode_paimants: selectedCategoryId }
      );
      fetchClients(); // Refresh categories after adding
  ...
  const handleEditModP = (categorieId) => {
    setErrors({});
    setSelectedCategoryId(categorieId.mode_paimants);
    setCategorie(categorieId.id);
  ...
  useEffect(() => {}, [selectedProductsData]);
        ...
        <Box component="main" sx={{ flexGrow: 1, p: 3, mt: 4 }}>
...

          {
            <div style={{ height: "125px", marginTop: "-15px" }}>
              <h5
                className="container-d-flex justify-content-start AjouteBotton"
                          ...
                          <a href="#" className="mx-5" key={index}>
                            ...
                            >
                              <img
                                src={
                                  category.logoP
                                    ? `http://localhost:8000/storage/${category.logoP}`
                                    : "http://localhost:8000/storage/secteur-activite.webp"
                                }
            ...
            <div style={{ marginTop: "0px" }}>
              ...
              >
                <Form className="col row" onSubmit={handleSubmit}>
                  ...
                  <Modal
                    show={showEditModalregions}
                    onHide={() => setShowEditModalregions(false)}
                  >
                    <Modal.Header closeButton>
                  ...
                  <Modal
                    show={showAddSecteur}
                    onHide={() => setShowAddSecteur(false)}
                  >
                    <Modal.Header closeButton>
                  ...
                  <Modal
                    show={showEditModalSecteur}
                    onHide={() => setShowEditModalSecteur(false)}
                  >
                    <Modal.Header closeButton>
                  ...
                  <Modal
                    show={showEditModalmod}
                    onHide={() => setShowEditModalmod(false)}
                  >
                    <Modal.Header closeButton>
                      <Modal.Title>Modifier mode de paiement</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                      <Form>
                        <Form.Group>
          ...
          <td colSpan="5" className="text-danger text-center">
            ...
            >
              <Form className="col row" onSubmit={handleSubmitSC}>
                ...
                <Modal
                  show={showAddCategorySite}
                  onHide={() => setShowAddCategorySite(false)}
                >
                  <Modal.Header closeButton>
                    <Modal.Title>Ajouter une Zone</Modal.Title>
                  </Modal.Header>
                ...
                <Modal
                  show={showEditModalSite}
                  onHide={() => setShowEditModalSite(false)}
                >
                  <Modal.Header closeButton>
                    <Modal.Title>Modifier une Zone</Modal.Title>
                  </Modal.Header>
                ...
                <Modal
                  show={showAddRegeinSite}
                  onHide={() => setShowAddRegeinSite(false)}
                >
                  <Modal.Header closeButton>
                    <Modal.Title>Ajouter une Region</Modal.Title>
                  </Modal.Header>
                ...
                <Modal
                  show={showEditModalregionsSite}
                  onHide={() => setShowEditModalregionsSite(false)}
                >
                  <Modal.Header closeButton>
                    <Modal.Title>Modifier une Region</Modal.Title>
                  </Modal.Header>
                ...
                >
                  ...
                  >
                    {filteredClients
                      ...
                      ?.map((client) => {
                        const rep = agent.find(
                          (agent) =>
                            agent.id === client.last_represantant?.id_agent
                        );
                        return (
                          <React.Fragment key={client.id}>
                            <tr>
                              ...
                              <td style={{ backgroundColor: "white" }}>
                                ...
                                <button
                                  onClick={() => toggleRowContact(client.id)}
                                  style={{
                                    border: "none",
                                    backgroundColor: "white",
                                  }}
                              ...
                              <td style={{ backgroundColor: "white" }}>
                                {highlightText(client.ice || "", searchTerm)}
                              </td>
                              <td style={{ backgroundColor: "white" }}>
                                {highlightText(
                                  client?.zone?.zone || "",
                            ...
                            {expandedRows.includes(client.id) &&
                              client.site_clients?.map((siteClient) => (
                                  ...
                                  <tr className="siteclient">
                                    ...
                                    <td>
                                      <img
                                        src={
                                          siteClient.logoSC
                                            ? `http://localhost:8000/storage/${siteClient.logoSC}`
                                            : "http://localhost:8000/storage/default_user.png"
                                        }
                                        alt="Logo"
                                        loading="lazy"
                                    ...
                                    <td colSpan={1}>
                                      {siteClient?.categorie || ""}
                                    </td>

                                    <td>
                                      {secteurClient.find(
                                        (agent) =>
                                          agent.id === siteClient.secteur?.id
                                      )?.secteurClient || ""}
                                    </td>
...
NomAgent|| ''}</td> */}
                                    ...
                                    <td>
                                      ...
                                      <FontAwesomeIcon
                                        onClick={() =>
                                          handleDeleteSiteClient(siteClient.id)
                                        }
                                        icon={faTrash}
                                        style={{
                                          color: "#ff0000",
                                  ...
                                  {expandedRowsContactSite.includes(
                                    siteClient.id
                                  ) &&
                                    siteClient.contact_site_clients && (
                                      <tr>
                                        <td
                                          colSpan="24"
                                          style={{
                                        ...
                                        >
                                          ...
                                          >
                                            ...
                                            <tbody>
                                              {siteClient.contact_site_clients?.map(
                                                (contact_clients) => (
                                                  <tr key={contact_clients.id}>
                                                    <td>
                                                      {contact_clients.name}
                                                    </td>
                                                    <td>
                                                      {contact_clients.prenom}
                                                    </td>
                                                    <td>
                                                      {
                                                        contact_clients.telephone
                                                      }
                                                    </td>
                                                    <td>
                                                      {contact_clients.email}
                                                    </td>
                                                  </tr>
                                                )
                                              )}
