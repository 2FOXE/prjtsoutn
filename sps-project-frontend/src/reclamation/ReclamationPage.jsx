...
const initDB = async () => {
  return openDB("ReclamationsDB", 3, {
    // 🔥 Change version to 3 (incremented)
...
const ReclamationPage = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // stockè la liste des recalamation chambre
  ...
  const renderExpandedRow = (item) => {
    ...
    const latestHistorique = item.historique
      .filter(
        (h) => h.description && h.description !== "Réclamation mise à jour"
      ) // ✅ Ignore generic updates
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if ([200, 201].includes(response.status)) {
        ...
        Swal.fire(
          "Succès!",
          `Réclamation ${id ? "modifiée" : "ajoutée"}`,
          "success"
        );
      }
    } catch (error) {
  ...
  const handleDeleteSelected = async () => {
    ...
    if (result.isConfirmed) {
      ...
      Swal.fire(
        "Supprimé!",
        "Les réclamations sélectionnées ont été supprimées.",
        "success"
      );
    }
  };
